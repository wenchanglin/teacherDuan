...
{
    NSLog(@"进度进度%f",[self fileTotalLength:url] == 0 ? 0.0 : 1.0 * HSDownloadLength(url) /  [self fileTotalLength:url]);
...
{
    NSLog(@"嚯嚯嚯嚯%ld",[[NSDictionary dictionaryWithContentsOfFile:HSTotalLengthFullpath][HSFileName(url)] integerValue]);
    ...
    if ([fileManager fileExistsAtPath:HSFileFullpath(url)]) {
        ...
        if ([fileManager fileExistsAtPath:HSTotalLengthFullpath]) {
            ...
            NSMutableDictionary *dict = [NSMutableDictionary dictionaryWithContentsOfFile:HSTotalLengthFullpath];
             NSLog(@"删除长度%@",HSTotalLengthFullpath);
